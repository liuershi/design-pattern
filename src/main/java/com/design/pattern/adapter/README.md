...
## Class diagram
![https://github.com/iluwatar/java-design-patterns/blob/master/adapter/etc/adapter.urm.png]()
...
适配器模式与**装饰模式**非常相似，他们主要的区别在于如下：

- 它们的目的不同，适配器模式是为了将已存在的类或对象复用，让原本客户端无法使用的类变为可以被客户端使用，其本质是利用现有的功能模块或代码；而装饰模式的目的是，在当前类现有的功能基础之上，额外增加新增的方法与功能，这是他们的本质区别。
